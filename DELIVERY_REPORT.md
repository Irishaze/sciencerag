...
- **OOD (out-of-distribution) thresholds in `/sciencerag/validate` are derived from the training set at call time, not fixed constants** — deliberately, because "is this design out of the range we've validated" is a statement about training coverage, which is expected to grow, not a fixed physical limit. As of today's 31-sample training set, the derived cutoffs are: `severity="blocking"` above **~13.32** (the single most extreme point ever validated, with no added margin — an earlier 1.5× margin was found to still be a linear function of that one point regardless of the multiplier, and to extend trust past validated territory in tension with what an OOD gate is for), `severity="warning"` above **~4.16** (the midpoint of the data's own largest gap, excluding the top point which blocking already owns — an earlier revision anchored this directly to the 2nd-highest historical score's value, ~4.49, which repeated the same "boundary sits on one real point" mistake the blocking margin already had; sitting inside the actual gap instead avoids that, and a first-principles re-check found the original percentile-rank cutoff landed inside the smooth, structureless part of the real score distribution, not at any gap the data itself shows — see `checks.py`'s `BLOCKING_MARGIN` docstring and the `warning_threshold` derivation for the full history). These numbers are **not hardcoded** and are **not meant to be manually retuned** — `tests/test_validate_route.py::test_ood_threshold_values_are_pinned_to_todays_training_data` pins them as a snapshot specifically so that retraining the surrogate on a larger dataset fails this test and forces a deliberate look at the new values, instead of either silently drifting (no test) or silently going stale (a hardcoded number nobody remembers to revisit) — confirmed by simulating a 32nd training sample and verifying the pinned test fails as intended. Recommended: re-run this test (and update the pinned numbers here after confirming they're sane) any time `tec_surrogate`'s training data changes.
